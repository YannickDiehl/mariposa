  ...
  if (length(var_names) == 2) {
    ...
    if (TRUE) {  # Always use SPSS-compatible manual calculations
      ...
      if (has_group) {
        ...
        # 2. WITHIN-SUBJECTS EFFECTS (Time main effect and Group × Time interaction)
        
        # Calculate weighted means for each Time × Group cell
        ...
        # Group × Time interaction sum of squares
        ss_interaction <- 0
        for (i in 1:nrow(cell_means)) {
        ...
        results_df <- tibble(
          Effect = c("Between-Subjects (Group)", "Within-Subjects (Time)", "Interaction (Group × Time)"),
      ...
      if (has_group) {
        ...
        # Extract Interaction (Group × Time) effect
        if (length(int_row) > 0) {
