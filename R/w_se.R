...
w_se <- function(data, ..., weights = NULL, na.rm = TRUE) {
  ...
  if (is.numeric(data) && !is.data.frame(data)) {
    ...
    if (!.are_weights(weights_vec) || !.validate_weights(weights_vec, verbose = FALSE)) {
      ...
      return(sd(x, na.rm = na.rm) / sqrt(length(x[!is.na(x)])))
    } else {
      ...
      dplyr::group_modify(~ {
        ...
        for (var_name in var_names) {
          ...
          if (is.null(weights_vec)) {
            # Unweighted calculation
            if (na.rm) x <- x[!is.na(x)]
            stat_val <- sd(x, na.rm = na.rm) / sqrt(length(x[!is.na(x)]))
  ...
  if (length(var_names) > 1) {
    ...
    for (i in seq_along(var_names)) {
      ...
      if (is_grouped) {
        ...
        for (j in 1:nrow(group_combinations)) {
          ...
          if (nrow(group_results) > 0) {
            ...
            if (!is.null(weights_name)) {
              row_data$weighted_se <- group_results[[var_name]][1]
              row_data$effective_n <- group_results[[paste0(var_name, "_eff_n")]][1]
            } else {
              row_data$se <- group_results[[var_name]][1]
        ...
        if (!is.null(weights_name)) {
          row_data$weighted_se <- results[[var_name]][1]
          row_data$effective_n <- results[[paste0(var_name, "_eff_n")]][1]
        } else {
          row_data$se <- results[[var_name]][1]
    ...
    if (!is.null(weights_name)) {
      final_results <- results %>%
        dplyr::mutate(
          weighted_se = !!rlang::sym(var_name),
          effective_n = !!rlang::sym(paste0(var_name, "_eff_n"))
        )
    } else {
      final_results <- results %>%
        dplyr::mutate(
          se = !!rlang::sym(var_name),
  ...
  if (is_grouped_data) {
    ...
    for (group_val in unique(x$results[[x$groups[1]]])) {
      ...
      if (!is.null(x$weights)) {
        print_df <- print_df %>%
          dplyr::select(dplyr::any_of(c("Variable", "weighted_se", "effective_n")))
        print_df$weighted_se <- round(print_df$weighted_se, digits)
        print_df$effective_n <- round(print_df$effective_n, 1)
      } else {
        print_df <- print_df %>%
          dplyr::select(dplyr::any_of(c("Variable", "se", "n")))
        print_df$se <- round(print_df$se, digits)
      }
    ...
    if (!is.null(x$weights)) {
      if ("Variable" %in% names(print_df)) {
        ...
        print_df <- tibble::tibble(
          ...
          weighted_se = round(print_df$weighted_se[1], digits),
