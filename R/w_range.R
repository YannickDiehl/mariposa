...
w_range <- function(data, ..., weights = NULL, na.rm = TRUE) {
  ...
  if (is.numeric(data) && !is.data.frame(data)) {
    ...
    # Calculate weighted or unweighted range
    if (!.are_weights(weights_vec) || !.validate_weights(weights_vec, verbose = FALSE)) {
      ...
      if (length(x) == 0) {
        ...
        # For range, weights don't change the calculation - it's still max - min
        result <- max(x) - min(x)
      }
    }
        ...
        for (i in seq_along(vars)) {
          ...
          if (is.null(weights_vec)) {
            ...
            if (length(x_clean) == 0) {
              ...
              # Range is max - min regardless of weights
              range_val <- max(x_clean) - min(x_clean)
              n_val <- sum(w_clean)
          ...
          results_list[[paste0(var_name, "_value")]] <- range_val
  ...
  if (length(vars) > 1 && !is_grouped) {
    ...
    for (i in seq_along(vars)) {
      ...
      if (value_col %in% names(results_df)) {
        ...
        row_data[[paste0(stat_prefix, "range")]] <- results_df[[value_col]]
    ...
    for (var in names(vars)) {
      ...
      if (value_col %in% names(results_df)) {
        ...
        hybrid_results[[paste0(stat_prefix, "range")]] <- results_df[[value_col]]
...
#'
#' @description
#' Prints a formatted summary of weighted range calculations, including range values,
#' sample sizes, and weights (if applicable). For grouped data, results are
#' displayed by group.
#'
#' @param x An object of class "w_range"
  ...
  if (x$grouped) {
    ...
    for (i in seq_len(nrow(groups))) {
      ...
      for (var in x$variables) {
        ...
        if (value_col %in% names(group_results)) {
          var_row <- data.frame(
            Variable = var,
            Range = round(group_results[[value_col]], digits),
            stringsAsFactors = FALSE
          )
    ...
    for (var in x$variables) {
      ...
      if (value_col %in% names(x$results)) {
        results_df$Range[results_df$Variable == var] <- round(x$results[[value_col]], digits)
