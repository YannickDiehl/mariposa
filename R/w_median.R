  ...
  if (length(var_names) > 1) {
    ...
    for (i in seq_along(var_names)) {
      ...
      if (is_grouped) {
        ...
        for (j in 1:nrow(group_combinations)) {
          ...
          if (nrow(group_results) > 0) {
            ...
            if (!is.null(weights_name)) {
              row_data$weighted_median <- group_results[[var_name]][1]
              row_data$effective_n <- group_results[[paste0(var_name, "_eff_n")]][1]
            } else {
              row_data$median <- group_results[[var_name]][1]
        ...
        if (!is.null(weights_name)) {
          row_data$weighted_median <- results[[var_name]][1]
          row_data$effective_n <- results[[paste0(var_name, "_eff_n")]][1]
        } else {
          row_data$median <- results[[var_name]][1]
    ...
    if (!is.null(weights_name)) {
      final_results <- results %>%
        dplyr::mutate(
          weighted_median = !!rlang::sym(var_name),
          effective_n = !!rlang::sym(paste0(var_name, "_eff_n"))
        )
    } else {
      final_results <- results %>%
        dplyr::mutate(
          median = !!rlang::sym(var_name),
  ...
  if (is_grouped_data) {
    ...
    for (group_val in unique(x$results[[x$groups[1]]])) {
      ...
      if (!is.null(x$weights)) {
        print_df <- print_df %>%
          dplyr::select(dplyr::any_of(c("Variable", "weighted_median", "effective_n")))
        print_df$weighted_median <- round(print_df$weighted_median, digits)
        print_df$effective_n <- round(print_df$effective_n, 1)
      } else {
        print_df <- print_df %>%
          dplyr::select(dplyr::any_of(c("Variable", "median", "n")))
        print_df$median <- round(print_df$median, digits)
      }
    ...
    if (!is.null(x$weights)) {
      if ("Variable" %in% names(print_df)) {
        ...
        print_df <- tibble::tibble(
          ...
          weighted_median = round(print_df$weighted_median[1], digits),
      ...
      if ("Variable" %in% names(print_df)) {
        ...
        print_df <- tibble::tibble(
          Variable = x$variables[1],
          median = round(print_df$median[1], digits),
