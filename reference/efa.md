...
`efa()` performs Exploratory Factor Analysis (EFA) using Principal
Component Analysis (PCA) to discover underlying patterns in your survey
items. This is the R equivalent of SPSS's
`FACTOR /EXTRACTION PC /ROTATION VARIMAX`.

For example, if you have 6 items measuring different attitudes, EFA can
reveal whether they group into 2-3 underlying dimensions (components).
...
- rotation:
  ...
  (oblique, allows correlated factors), or `"none"`.

- extraction:

  Extraction method. Currently only `"pca"` (Principal Component
...
- pattern_matrix:
  ...
  Pattern matrix (oblimin only, NULL otherwise)

- structure_matrix:

  Structure matrix (oblimin only, NULL otherwise)

- factor_correlations:

  Factor correlation matrix (oblimin only, NULL otherwise)
...
efa(survey_data,
    ...
    rotation = "oblimin")
#> 
#> Exploratory Factor Analysis (Oblimin) Results
#> ---------------------------------------------
#> - Variables: political_orientation, environmental_concern, life_satisfaction, trust_government, trust_media, trust_science
#> - Extraction: Principal Component Analysis
