...
#' Perform t-tests with support for weights, paired tests, and grouped data
#'
#' @description
#' \code{t_test()} performs one-sample, two-sample, and paired t-tests on one or more 
...
#'   For paired t-tests, provide exactly 2 variables.
...
#'   Cannot be used with \code{paired = TRUE}.
...
#' @param paired Logical indicating whether to perform paired t-tests. Requires 
#'   exactly 2 variables and no grouping variable. Default is \code{FALSE}.
#'   Can be combined with grouped data (via \code{group_by()}) to perform paired 
...
#'   hypothesized difference in means. For paired tests, this is the hypothesized 
#'   mean difference. Default is \code{0}.
...
#'   \item{paired}{Logical indicating if paired tests were performed}
...
#' \describe{
#'   \item{Variable}{Name of the analyzed variable (or "var1 vs var2" for paired tests)}
#'   \item{t_stat}{t-statistic}
#'   \item{df}{Degrees of freedom}
#'   \item{p_value}{p-value}
#'   \item{mean_diff}{Mean difference (group1 - group2 for two-sample, diff for paired)}
...
#' analyses for each group combination. For paired tests with grouped data, 
#' the function performs proper paired comparisons within each group.
#' 
#' ## SPSS Compatibility
...
#' # Paired t-test
#' survey_data %>%
#'   t_test(pre_test, post_test, paired = TRUE)
#' 
#' # Weighted analysis
#' survey_data %>%
...
#'   t_test(outcome1, group = group)
#' 
#' # Grouped paired t-test (paired comparison within each gender)
#' survey_data %>%
#'   group_by(gender) %>%
#'   t_test(pre_test, post_test, paired = TRUE)
#' 
...
#'
#' @export
t_test <- function(data, ..., group = NULL, weights = NULL, paired = FALSE, 
  ...
  if (paired) {
    if (length(var_names) != 2) {
      stop("For paired t-test: provide exactly 2 variables")
    }
    if (!is.null(g_name)) {
      stop("Paired t-test cannot be combined with grouping variable. Use group_by() for grouped paired tests.")
    }
    if (!is.null(w_name)) {
      stop("Weighted paired t-test is not currently supported")
    }
  }
  ...
  perform_paired_t_test <- function(data, var1_name, var2_name) {
    ...
    hedges_j <- 1 - (3 / (4 * (n - 1) - 1))
    hedges_g <- cohens_d * hedges_j
    
    # Prepare group statistics for display
    group_stats <- list(
      var1 = list(name = var1_name, mean = mean(x1), n = length(x1)),
      var2 = list(name = var2_name, mean = mean(x2), n = length(x2)),
      differences = list(mean = mean(differences), sd = sd(differences))
    )
    ...
    return(list(
      ...
      mean_diff = test_result$estimate,  # This is the mean of differences
      conf_int = test_result$conf.int,
      cohens_d = list(cohens_d = cohens_d, hedges_g = hedges_g, glass_delta = NA),
      group_levels = c(var1_name, var2_name),
      group_stats = group_stats,
      equal_var_result = NULL,
      unequal_var_result = NULL,
      is_weighted = FALSE
    ))
  }
  ...
  if (is_grouped) {
    ...
    results_df$equal_var_result <- list(test_result$equal_var_result)
    results_df$unequal_var_result <- list(test_result$unequal_var_result)
    results_df$is_weighted <- test_result$is_weighted
...
print.t_test_results <- function(x, digits = 3, ...) {
  # Determine test type based on paired and weights (Template Standard)
  test_type <- if (x$paired) {
    "Paired t-Test Results"
  } else if (!is.null(x$weight_var) || !is.null(x$weights)) {
  ...
  if (is_grouped_data) {
    ...
    for (i in seq_len(nrow(groups))) {
      ...
      for (j in seq_len(nrow(group_results))) {
        ...
        if (x$paired && !is.null(stats)) {
          # Paired t-test: show both variables and differences
        ...
        } else if (!is.null(stats) && !is.null(stats$group1)) {
        ...
        if (!is.na(cohens_d_val)) {
          effect_df <- .create_effect_size_df(var, cohens_d_val, hedges_g_val, glass_delta_val, x$paired)
    ...
    for (i in seq_len(nrow(valid_results))) {
      ...
      if (x$paired && !is.null(stats)) {
        ...
        cat("\n")
      } else if (!is.null(x$group) && !is.null(stats) && !is.null(stats$group1)) {
        # Group comparison: show group means
        cat(sprintf("  %s: mean = %.3f, n = %.1f\n", 
                    stats$group1$name, stats$group1$mean, stats$group1$n))
        cat(sprintf("  %s: mean = %.3f, n = %.1f\n", 
      ...
      if (!is.na(cohens_d_val)) {
        effect_df <- .create_effect_size_df(var_name, cohens_d_val, hedges_g_val, glass_delta_val, x$paired)
