...
| `row_means()` | Create mean indices across items |
...
`row_means()` works inside `mutate()`, computing the row-wise mean across items.
...
3. `row_means()` computes mean indices for use in further analyses
